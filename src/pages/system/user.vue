      ...
      <div class="right-content">
        ...
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" row-class-name="row_class" border
          ...
          <el-table-column align="center" label="授权角色名称" show-overflow-tooltip>
            <template slot-scope="scope">
              <span
                :class="scope.row.rolename==null?'red':'green'">{{scope.row.rolename==null?'未授权': scope.row.rolename}}</span>
            </template>
          </el-table-column>
        ...
        <el-dialog :title="title" custom-class="dialog" :modal="modal" :before-close="resetForm"
          :visible.sync="showAppDetail" width="400px" :close-on-click-modal="modal" @close="resetForm">
      ...
      <div class="dialog_warp">
        <el-form ref="form" label-width="120px" class="demo-ruleForm">
          <el-form-item label="导入文件" style="width:100%;">
            <el-upload action="" class="upload-demo" ref="uploadleaders" :auto-upload="false" :limit="1"
              accept=".xlsx, .xls" :http-request="uploadSectionFile">
